...
function endScene() {
	...
	//Clean up any eventual spine viewers that have been preloaded
	//Also remove spine players from the "preloading" container element, if some are still there
	emptySpinePreload();
...
function buildSceneViewer() {
	...
	//Textbox and name UI starts as hidden by default.
...
async function runSceneCommands() {
	...
	for (let command of scene.commands) {
		...
		switch (tag) {
			...
			//Ex: <SPINE_ACTOR>0,char_00001501_01,6,CENTER, CENTER, IN, linear, 1000
			case "SPINE_ACTOR":

				let _actorIdx = Number(data.split(",")[0]);
				let _actorFilename = data.split(",")[1].trim();
				let _actorAnimationName = data.split(",")[2].trim();
				let _actorPos = data.split(",")[3].trim();
				let _actorPos2 = data.split(",")[4].trim();
				let _actorMoving = data.split(",")[5].trim();
				let _actorTiming = data.split(",")[6].trim();
				let _actorDur = Number(data.split(",")[7]);

				//If this is the first time the spine actor is seen, add it to the actors' wrapper container and save a reference to it
				//The actor never gets removed from this wrapper until the scene ends, it is only shown/hidden
				if (!scene.actors[_actorIdx]) {
					scene.actors[_actorIdx] = preload.spines[_actorFilename]; //Spine player element (div)
					scene.elements.actors.appendChild(scene.actors[_actorIdx]);
				}
				//Save the active current spine, since other spine commands after, like: "SPINE_PLAY", will act on that
				//preload.currentSpine=_actorFilename;

				//else if(_actorMoving == "NONE")
				if (_actorMoving == "NONE") {
					//Change animation (static facial expression here)
					scene.actors[_actorIdx].spineWidget.play();
					scene.actors[_actorIdx].spineWidget.setAnimation(_actorAnimationName);
					//Since the animations here are just static facial expressions, there is no need to run the animation continuously, so just pause it immediately again
					scene.actors[_actorIdx].spineWidget.pause();

					//scene.actors[actorIdx].style.backgroundImage = "url('" + createImagePath(actor) + "')";
					setActorPosition(scene.actors[_actorIdx], _actorPos2, _actorPos);
				}
				/*
				if(_actorMoving == "IN")
				{
					...
					animateElement(scene.actors[_actorIdx], _actorDur, "fade-in", true, _actorTiming);
					scene.skippableAnimation = false;
				}
				*/
				/*
				else if(_actorMoving == "OUT")
				{
					//Change animation (static facial expression here)
					scene.actors[_actorIdx].spineWidget.setAnimation(_actorAnimationName);

					if(scene.actors[_actorIdx].style.opacity == "1")
					{
						if(_actorDur > 0)
						{
						  animateElement(scene.actors[_actorIdx], _actorDur, "fade-out", true, _actorTiming);
						  scene.skippableAnimation = false;
						}
						//If duration is 0, instantly hide the actor
						else
						{
							scene.actors[_actorIdx].style.opacity=0;
						}
					}
				}
				*/
				/*
				else if(_actorMoving == "NONE" && actorDur > 0)
				{
					...
					setActorPosition(scene.actors[actorIdx], actorPos2, actorPos);
					scene.skippableAnimation = false;
				...
				*/
					...
					console.log("Unknown spine actor movement: " + _actorMoving);
				}
			...
			case "SPINE_ACTOR_DISPLAY":

				let _actorIdx1 = Number(data.split(",")[0]);
				let _displayState = data.split(",")[1].trim();

				//show actor if hidden
				if (_displayState === "SHOW") {
					scene.actors[_actorIdx1].style.opacity = 1;
				}
				//hide actor if visible
				else if (_displayState === "HIDE") {
					scene.actors[_actorIdx1].style.opacity = 0;
				}
				else {
					console.log("Unknown spine actor display state: " + _displayState);
				}
			...
			case "ACTOR":
				...
				if (actorMoving == "IN") {
					...
					if (scene.actors[actorIdx].style.opacity == "1") {
						...
						//If duration is 0, instantly hide the actor
							...
							scene.actors[actorIdx].style.opacity = 0;
						}
					}
				}
				...
				//My addition, show hidden actor
				else if (actorMoving == "SHOW") {
					scene.actors[actorIdx].style.opacity = 1;
				}
				//My addition, hide actor if visible
				else if (actorMoving == "HIDE") {
					scene.actors[actorIdx].style.opacity = 0;
				}
				else {
			...
			case "BG":
				...
				let otherBgElem = scene.alt.bg ? scene.elements.background : scene.elements.backgroundAlt;
				...
				if (bgImg.toLowerCase() == "black") {
					...
					otherBgElem.style.zIndex = "1";

					// Clear other background to prevent layering
					otherBgElem.style.backgroundImage = "";
					otherBgElem.style.opacity = "0";
			...
			case "EV":
				...
				let otherElem = scene.alt.bg ? scene.elements.background : scene.elements.backgroundAlt;
				...
				// Clear other background to prevent layering of pre-composed images
				if (evDur == 0) {
					killChildren(otherElem);
			...
			case "PAUSE":
				if (!scene.script[scene.index].startsWith("<PAUSE>")) {
					...
					//console.log(scene.script);
					//console.log(scene.index);
					//console.log(scene.script[scene.index]);
					//console.log("running");
					scene.paused = true;
					scene.empty.textBox = true;
					//console.log(scene.empty.textBox);
			...
			//This assumes there are no commas in the label and text, which there probably isn't
			...
			case "SELECT":
				...
				optBtn.innerHTML = '<div class="scene-choice-btn-left"></div><div class="scene-choice-btn-mid">' + txt + '</div><div class="scene-choice-btn-right"></div>';
				//optBtn.innerHTML='<tr class=""><td class="">'+txt+'</td></tr>';
			...
			case "VOICE_PLAY":
				...
				// In the script the VOICE_PLAY command sometimes comes before the text command, which is the normal way
				// but sometimes it comes after the text command, and in that case we do this as a quick fix
				// to add the voice to the backlog too.
				// This assumes the player never speaks and that every dialog is accompanied by a voice line too.
				let sceneBacklogItem = scene.elements.backlog.querySelector(".scene-backlog-item:last-child");
				if (sceneBacklogItem !== null) {
					if (sceneBacklogItem.querySelector(".scene-backlog-item-voice") === null) {
						let sceneBacklogItemVoice = document.createElement("div");
						sceneBacklogItemVoice.classList = "scene-backlog-item-voice";

						sceneBacklogItemVoice.setAttribute("voice", data.trim());
						sceneBacklogItemVoice.setAttribute("path", scene.current.voice.src);

						sceneBacklogItemVoice.addEventListener("click", function (e) {
							e.stopPropagation();
							scene.current.voice.pause();
						...
						sceneBacklogItem.prepend(sceneBacklogItemVoice);
					}
				}
			...
			case "": //Doesn't have a tag so most likely scene text.
				...
				// My additions, add the text directly
				scene.elements.textBoxText.innerHTML = data;
				...
				//Replace newlines in text script with <br />
				scene.elements.textBoxText.innerHTML = scene.elements.textBoxText.innerHTML.replace(/\n/gi, "<br />");
...
function addRPGXText() {
	...
	//Replace <param=PlayerName> with the players name, if any exist.
	//txt = txt.replace(/<param=PlayerName>/g, 'Hero'); 
...
async function runNecroCommand() {
	...
	switch (data[0]) {
		...
		case "seplay":
			scene.current.se.pause();
			await loadAudioNow(constructSEAudioPath(data[1].trim()), "se");
			switchAudio("se", scene.current.se, scene.prev.se, preload.audio.se);
	...
	async function necroVoice(voice) {
		if (!voice.includes("_i_men")) {
			scene.voicedLine = true;
			scene.current.voice.pause();
			await loadAudioNow(constructVoiceAudioPath(voice.trim()), "voice");
			switchAudio("voice", scene.current.voice, scene.prev.voice, preload.audio.voice);
...
async function runOtogiCommand() {
	...
	if (cmd.Voice !== "") {
		scene.voicedLine = true;
		scene.current.voice.pause();
		await loadAudioNow(constructVoiceAudioPath(cmd.Voice.trim()), "voice");
		switchAudio("voice", scene.current.voice, scene.prev.voice, preload.audio.voice);
	...
	if (cmd.BGM != null) {
		scene.current.bgm.pause();
		await loadAudioNow(constructBGMAudioPath(cmd.BGM.trim()), "bgm");
		switchAudio("bgm", scene.current.bgm, scene.prev.bgm, preload.audio.bgm);
	...
	switch (type) {
		...
		case "TXT":
			...
			if (name === "del" || (name === "" && text === " ")) {
				...
				// Voice
				if (src) {
					let fn = src.split("/")[src.split("/").length - 1].replace(/\.[^/.]+$/, "");
					if (fn != undefined && !fn.includes("non_resource")) {
						scene.voicedLine = true;
						scene.current.voice.pause();
						await loadAudioNow(constructVoiceAudioPath(fn), "voice");
						switchAudio("voice", scene.current.voice, scene.prev.voice, preload.audio.voice);
...
function displayChoice() {
	...
	scene.elements.choiceBox.style.zIndex = "8";
}
...
//My addition, add posY too(position vertically)
function setActorPosition(actor, posX, posY) {
	switch (posX) {
		case "RIGHT_DOUBLE":
			//actor.style.left = "120px";
			actor.style.backgroundPositionX = "200%";
			break;
		case "RIGHT":
			actor.style.backgroundPositionX = "right";
			break;
		case "RIGHT_MIDDLE":
			actor.style.backgroundPositionX = "75%";
			break;
		case "LEFT_DOUBLE":
			actor.style.backgroundPositionX = "-100%";
			break;
		case "LEFT":
			//actor.style.left = "-440px";
			actor.style.backgroundPositionX = "left";
			break;
		case "LEFT_MIDDLE":
			actor.style.backgroundPositionX = "25%";
			break;
		case "CENTER":
			//Default position
			actor.style.backgroundPositionX = "center";
	...
	switch (posY) {
		case "TOP":
			//default position
			actor.style.backgroundPositionY = "top";
			break;
		case "TOP_NEAR":
			actor.style.backgroundPositionY = "12%";
			break;
		case "TOP_MIDDLE":
			actor.style.backgroundPositionY = "25%";
			break;
		case "CENTER":
			//center the character on screen
			actor.style.backgroundPositionY = "center";
			break;
		case "BOTTOM":
			actor.style.backgroundPositionY = "bottom";
			break;
		case "BOTTOM_MIDDLE":
			actor.style.backgroundPositionY = "75%";
			break;
		case "BOTTOM_DOUBLE":
			actor.style.backgroundPositionY = "200%";
			break;
		default:
			//Untested
			var parts = posY.split("=");
			if (parts[0] === 'y') {
				var top = parseInt(actor.style.top);

				var move_y = -1 * (+parts[1]);
