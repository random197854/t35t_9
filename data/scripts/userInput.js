...
function toggleSceneAutoMode(){
	if(scene.mode != 1 && canProgress(false)){
		...
		scene.elements.textBoxIcon.style.backgroundImage = "url('data/ui/text_icon.png')"
		sceneAutoMode();
...
async function chooseScene(id){
	...
	if(prefs.scene.eng && sceneData[id].SCRIPTS.PART1.TRANSLATIONS != null){
		...
		if(tls.length > 1){
			...
			if(typeof tls[0].SCRIPT == "string"){
				let req = await fetch("data/scripts/data/" + tls[0].SCRIPT);
				scene.script = await req.json();
			} else {
		...
		if(typeof sceneData[id].SCRIPTS.PART1.SCRIPT == "string"){
			let req = await fetch("data/scripts/data/" + sceneData[id].SCRIPTS.PART1.SCRIPT);
			scene.script = await req.json();
		} else {
...
async function tlSelect(idx){
	killChildren(main.elements.tlChoiceBox);
	let choice = sceneData[scene.id].SCRIPTS.PART1.TRANSLATIONS[idx]
	if(typeof choice.SCRIPT == "string"){
		let req = await fetch("data/scripts/data/" + choice.SCRIPT);
		scene.script = await req.json();
	} else {
