...
function nextCG() {
	...
	//	nextCGScene();
	//}
	if (cgViewer.index >= cgViewer.imgQ.length - 1) {
		nextCGScene();
		cgViewer.index = 0;
	}
	else {
	...
	//If it is not an event image, like a background image, don't display it and just advance to the next cg. New addition
	if (cgViewer.imgQ[cgViewer.index].substring(0, 6) !== "still_") {
		nextCG();
	}
...
function displayCG() {
	...
	if (cgViewer.rpgx) {
		...
		if (cgViewer.scene[0] == "c") {
			cgElem.style.background = "black url('" + createImagePath(curCG, "") + "') no-repeat center";
			...
			if (ext == "webm") {
				...
				vid.height = 720;
				vid.loop = true;
				vid.src = createImagePath(curCG, "");
				vid.classList = "viewer-main-class viewer-video";
				cgElem.appendChild(vid);
				vid.load();
				vid.play();
			} else {
				cgElem.style.background = "black url('" + createImagePath(curCG, "") + "') no-repeat center/960px 720px";
			}
