var main = {
	...
	info:{
		rpgx_h:{
			date:"",
			desc:"Aku No Onna Kanbu - Perigee New Moon Viewer - 1.4",
		},
		rpgx_story:{
			date:"< 2020-08-18",
...
function checkData(){
	if(typeof sceneData !== "undefined"){
		...
		console.log(`Aku No Onna Kanbu - Perigee New Moon H Scenes\nDescription: ${main.info.rpgx_h.desc}`);
	}
...
function stretchFrame(){
	if(prefs.scene.textBoxFullUnder){
		...
		main.elements.contents.style.height = "914px";
		//main.elements.contents.style.backgroundSize = "1002px 918px"
		main.elements.contents.style.backgroundSize = "1322px 962px";
	}
}
...
function contractFrame(){
	...
	main.elements.contents.style.backgroundSize = "1322px 768px";
}
...
function loadSceneViewer(){
	...
	if(window.getComputedStyle(main.elements.viewer).getPropertyValue("transition-duration")!=="0s" && !prefs.viewer.loadingScreen)
	{
